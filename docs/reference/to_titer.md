...
- model:
  ...
  \- either the name of a built-in model to fit standard curve or a
  named list of 2 functions for "mod" and "quantify_ci"
...
- negative_control:
  ...
  \- if TRUE, output tibble will include the result for negative
  controls
...
- antitoxin_df:
  ...
  list of \`data.frame\`s containing the results for antitoxins from
  each plate
