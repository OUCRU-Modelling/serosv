...
Let's consider a transmission model consists of 3 compartments: susceptible (S), infected (I), recovered (R)
...
To simulate a basic SIR model, use `sir_basic_model()` and specify the following parameters
...
To simulate an SIR model with constant FOI, use `sir_static_model()` and specify the following parameters
...
To simulate a SIR model with subpopulations, use `sir_subpops_model()` and specify the following parameters
...
Extends on SIR model with 2 additional compartments: maternal immunity (M) and exposed period (E)
...
To simulate a MSEIR model, use `mseir_model()` and specify the following parameters
