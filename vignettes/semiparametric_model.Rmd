...
Where $\eta = [\eta(a_i) ... \eta(a_N) ]^T$, $\beta = [\beta_0 \beta_1 .... \beta_p]^T$, and $\textbf{u} = [u_1 u_2 ... u_k]^T$ are the regression coefficients with corresponding design matrices
...
Refer to Chapter `8.2.1` of the book by @Hens2012 for a more detailed explanation of the method.
...
Looking back at \@ref(eq:penlikelihood), a constraint for $\textbf{u}$ would be $\Sigma_ku_k^2 < C$ for some positive value $C$

This is equivalent to choosing $(\beta, \textbf{u})$ to maximise \@ref(eq:penlikelihood) with $D = diag(\textbf{0}, \textbf{1})$ where $\textbf{0}$ denotes zero vector length $p+1$ and $\textbf{1}$ denotes the unit vector of length $K$
...
Refer to Chapter `8.2.2` of the book by @Hens2012 for a more detailed explanation of the method.
...
plot(
  glmm, foi_ci = TRUE, 
  ncpus = 2 # limit used cores for vignette building
)
```
