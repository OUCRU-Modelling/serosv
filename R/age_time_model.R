...
age_time_model <- function(data,
                           ...
                           age_correct=F, le=512, monotonize_method = "pava"){
  ...
  monotonize_data <- \(dat, grp){
    ...
    if(monotonize_method == "scam"){
      out <- tryCatch(
        {
          mod <- scam(y ~ s(mean_time, bs = "mpi"), data=dat,family = betar)
  ...
  if(age_correct == FALSE){
    # simulate data + monotonize data using scam/pava depending on user's input
    monotonize_out <- gam_mods %>%
    ...
    # ----- monotonize cohort-stratifed prevalence over time using scam/pava ----
    monotonize_out <- dat_monotonize %>%
      filter(
    ...
    cohort_age_mapping <- dat_monotonize %>%
