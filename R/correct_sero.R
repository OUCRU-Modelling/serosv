...
#' correct_sero(data$age, pos = data$pos, tot = data$tot)
correct_sero <- function(age, pos=NULL, tot=NULL, status=NULL,
  ...
  fit <- rstan::sampling(stanmodels$sero_correction, data=data, chains=chains,warmup=warmup, iter=iter)
